...
case class RetrievedOSVersion private[swiftbar4s](major:Int, minor:Int, patch:Int) {
    infix def is (version:OSVersion) = this.major == version.major && version.minors.contains(this.minor)
    infix def isNot (version:OSVersion) = !is(version)
    infix def < (version:OSVersion) = this.major < version.major || (this.major == version.major && this.minor < version.minors.min)
